        ...
        if (metaInfo.sync) {
          ...
          component = 'Loadable({ page:' + JSON.stringify(reducerKey) + ',loading: Loading, loader: () =>' +
            ...
            config.reducerInject + '[' + JSON.stringify(reducerKey) + '] = '+ (
              config.reducerDecorator ? config.reducerDecorator + '(reducer, ' + JSON.stringify(reducerKey) + ');':
  ...
  traverseTree(components, function (value, key, hasChild) {
    ...
    if (!hasChild && (shouldBuild || forceSync)) {
      ...
      resultStr += '"' + reducerKey + '": ' + (
        (config.reducerDecorator && identifier !== '""') ? config.reducerDecorator + '(' + identifier + ', ' + JSON.stringify(reducerKey) + ')':
          identifier) +',\n';
    }
  }, function () {
    currentPath.pop();
